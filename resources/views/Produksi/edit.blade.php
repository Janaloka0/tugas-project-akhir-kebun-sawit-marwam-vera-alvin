        ...
        <form action="{{ route('produksi.update', $produksi->id) }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <select class="form-select" name="role" id="role" required>
                    <option value="admin" {{ $produksi->role == 'admin' ? 'selected' : '' }}>Admin</option>
                    <option value="petugas kebun" {{ $produksi->role == 'petugas kebun' ? 'selected' : '' }}>Petugas Kebun</option>
                    <option value="manajer" {{ $produksi->role == 'manajer' ? 'selected' : '' }}>Manajer</option>
                </select>
            </div>
